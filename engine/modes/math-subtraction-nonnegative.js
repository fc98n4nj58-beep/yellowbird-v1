...
function buildPools(params) {
  ...
  const A = clampRange(params.aMin, params.aMax, 1, 20);
  const B = clampRange(params.bMin, params.bMax, 1, 20);
...
function buildUniquePairs(aPool, bPool, nonneg) {
  const pairs = [];
  const seen = new Set();

  for (const a0 of aPool) {
    for (const b0 of bPool) {
      let a = a0;
      let b = b0;

      if (nonneg && b > a) [a, b] = [b, a];
...
function generate(params = {}) {
  ...
  const nonneg = (params.nonneg ?? "1").toString() !== "0";

  const pool = buildUniquePairs(aPool, bPool, nonneg);
  if (!pool.length) {
    const err = new Error("No valid questions can be generated with these constraints.");
    err.status = 400;
    throw err;
  }
  ...
  return {
    meta: {
      modeId: "math.subtraction.nonnegative",
      subject: "math",
      title: nonneg ? "Subtraction Practice (Non-negative)" : "Subtraction Practice",
